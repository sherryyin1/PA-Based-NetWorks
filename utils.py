...
class CameDataset(Dataset):
    def __init__(self, img_labels, img_dir, is_train=True, transform=None):
        #self.img_labels = pd.read_csv(annotations_file, index_col=0, dtype={"patient":"str"})
        #self.img_labels = self.img_labels.loc[self.img_labels["node"]==0]
        #self.img_labels = self.img_labels.sample(frac=1)
        #self.img_labels = self.img_labels.reset_index(drop=True)

        # train_size = int(len(self.img_labels) * 0.8)
        self.img_labels = img_labels.reset_index(drop=True)
        """
        if is_train:
            self.img_labels = self.img_labels[:train_size]
            self.img_labels = self.img_labels.reset_index(drop=True)
        else:
            self.img_labels = self.img_labels[train_size:]
            self.img_labels = self.img_labels.reset_index(drop=True)
    ...
    def __getitem__(self, idx):
        patient = self.img_labels.loc[idx, "patient"]
        node = self.img_labels.loc[idx, "node"]
        x = self.img_labels.loc[idx, "x_coord"]
        y = self.img_labels.loc[idx, "y_coord"]
        img_path = self.img_dir + f"patches/patient_{patient}_node_{node}/patch_patient_{patient}_node_{node}_x_{x}_y_{y}.png"
        image = Image.open(img_path).convert('RGB')

        label = self.img_labels.loc[idx, "tumor"]
        if self.transform:
